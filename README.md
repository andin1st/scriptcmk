...
###### **A. Server Installation (Docker + Dockge)**
Jalankan perintah berikut di terminal server Linux untuk memasang Docker Engine (dengan *systemd auto-start*), Dockge Stack Manager, dan Checkmk Community Edition Server:
```bash
...
```
*Untuk kebutuhan deployment massal non-interaktif di Linux, Anda dapat melewatkan parameter konfigurasi Server secara langsung:*
```bash
curl -sSfgL https://raw.githubusercontent.com/andin1st/scriptcmk/main/linux/install.sh | sudo bash -s -- -s 192.168.1.100 -d cmk -v 2.5.0p9
```

###### **C. Windows Host Agent**
Jalankan perintah PowerShell berikut sebagai Administrator pada host Windows target:
```powershell
...
###### **2. cpu_info (.sh / .ps1)**
* **Fungsi**: Utilisasi beban CPU, spesifikasi core/thread, kecepatan clock speed, dan suhu sensor fisik.
* **Linux Output**: `0 "CPU_Info" - Spesifikasi : Intel Core i3 13100 | Clock Speed : 3.4Ghz | Core/Thread : 4/8 | CPU Load : 12% | CPU Temperature: 46 Celcius`
* **Windows Output**: `0 "CPU_Info" - Spesifikasi : Intel Core i3 13100 | Clock Speed : 3.4Ghz | Core/Thread : 4/8 | CPU Load : 12% | CPU Temperature: 46 Celcius`
...
* **Output (Identik)**: `0 "Info_OS" - OK - OS: Microsoft Windows 11 Pro | Kernel: 10.0.22631 | Arch: 64-bit | License: Activated (Licensed) ❘ Checked At: 2026-08-22 16:00:00`
