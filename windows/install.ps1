...
# 6. Setup RAM Health (Pengujian Memtester / Memory Diagnostik Asinkron - Mingguan/2-Mingguan)
Write-Host "[-] Menyiapkan penjadwalan uji kesehatan RAM..." -ForegroundColor Yellow
...
# Hitung 20% dari Free RAM saat ini secara dinamis
try {
...
Add-Content -Path $LogFile -Value "Menjalankan memtester dengan alokasi ${SampleMB}MB (20% Free RAM)..."

# Menjalankan stress memory sederhana menggunakan alokasi objek .NET
try {
    Write-Output "Mengalokasikan memori untuk testing ($SampleMB MB)..."
    $testBytes = [int64]$SampleMB * 1MB
    $testArray = New-Object Byte[] $testBytes
