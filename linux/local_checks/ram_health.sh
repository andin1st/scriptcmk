...
if grep -q "STATUS: SUCCESS" "$LOG_FILE"; then
    ...
    echo "0 \"Health_RAM\" - Status Memory: Ok, tidak ditemukan error saat pengecekan | Sample Pengujian : $sample_size | $timestamp"
elif grep -q "STATUS: FAILED" "$LOG_FILE"; then
    # Status 2 = CRITICAL
    echo "2 \"Health_RAM\" - Status Memory: Critical, ditemukan error kerusakan memory! | Sample Pengujian : $sample_size | $timestamp"
else
    # Status 1 = WARNING (Sedang berjalan / log belum lengkap)
    echo "1 \"Health_RAM\" - Status Memory: Warning, pengujian RAM sedang berlangsung | Sample Pengujian : $sample_size | $timestamp"
fi
